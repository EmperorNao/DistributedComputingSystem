...
void network::ConnectionConfig::load(config::Config &config) {
    ...
    if (str_type != "Master" and str_type != "Client") {
        throw std::domain_error(
                "Error in ConnectionConfig, "
                "type must be 'Client' or 'Master' "
                "but get '" + str_type + "'");
    }

    type = str_type == "Master" ? ManagerType::Server : ManagerType::Client;
...
void network::ConnectionConfig::save(config::Config &config) const {
    ...
    config.set("type", type == ManagerType::Server ? "Master" : "Client");
